...
def _executar_chamada_api_ibge(config: IbgeQueryConfig) -> pd.DataFrame:
    ...
    Args:
        config (IbgeQueryConfig): Configuração da consulta ao IBGE.
...
def extrair_dados_ipca(config: IbgeQueryConfig = None) -> pd.DataFrame:
    ...
    Args:
        config (IbgeQueryConfig, optional): Configuração da consulta ao IBGE. Padrão é instanciar com os valores default.
